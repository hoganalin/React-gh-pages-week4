...
function ProductModal({ modalMode,tempProduct,getProducts,isOpen,setIsOpen}) {
  ...
  const handleUpdateProduct = async () => {
    ...
    useEffect(() => {
      // console.log(productModalRef.current) /把dom印出來
      new Modal(productModalRef.current, {
        backdrop: false
      });

      //getInstance 是 Modal 的一個方法，用來獲取與某個 DOM 元素相關聯的模態框實例。
        ...
        <div className="modal-dialog modal-dialog-centered modal-xl">
          <div className="modal-content border-0 shadow">
            ...
            <div className="modal-footer border-top bg-light">
              <button type="button" onClick={handleCloseProductModal} className="btn btn-secondary">取消</button>
              <button type="button" onClick={handleUpdateProduct} className="btn btn-primary">確認</button>
            </div>
          </div>
        </div>
      </div>
